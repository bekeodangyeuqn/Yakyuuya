...
const Search = (props) => {
  ...
  const filterSearch = ({
    ...
    if (min) query.min ? query.min : query.min === 0 ? 0 : query.min;
    if (max) query.max ? query.max : query.max === 0 ? 0 : query.max;
...
export async function getServerSideProps({ query }) {
  ...
  const products = productDocs.map(db.convertDocToString);
